...
{		
	...
	/* 配置 LED_Block 引脚 */
	RCC_APB2PeriphClockCmd ( macLED_Block_GPIO_CLK, ENABLE ); 															   
	GPIO_InitStructure.GPIO_Pin = macLED_Block_GPIO_PIN;	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_Init ( macLED_Block_GPIO_PORT, & GPIO_InitStructure );	 

	/* 配置 LED_Circle 引脚 */
	RCC_APB2PeriphClockCmd ( macLED_Circle_GPIO_CLK, ENABLE ); 															   
	GPIO_InitStructure.GPIO_Pin = macLED_Circle_GPIO_PIN;	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_Init ( macLED_Circle_GPIO_PORT, & GPIO_InitStructure );			


	/* 配置 LED_Orange 引脚 */
	RCC_APB2PeriphClockCmd ( macLED_Orange_GPIO_CLK, ENABLE ); 															   
	GPIO_InitStructure.GPIO_Pin = macLED_Orange_GPIO_PIN;	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_Init ( macLED_Orange_GPIO_PORT, & GPIO_InitStructure );	 


	/* 配置 LED_White 引脚 */
	RCC_APB2PeriphClockCmd ( macLED_White_GPIO_CLK, ENABLE ); 															   
	GPIO_InitStructure.GPIO_Pin = macLED_White_GPIO_PIN;	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_Init ( macLED_White_GPIO_PORT, & GPIO_InitStructure );	
}
...
void LED_Out(int8_t ledName, int8_t ledState)
{
	...
	switch(ledName)
	{
		case LED_Name_4G:
			...
			break;
		case LED_Name_Block:
			if(ledState)	macLED_Block_ON();
			else			macLED_Block_OFF();
			break;
		case LED_Name_Circle:
			if(ledState)	macLED_Circle_ON();
			else			macLED_Circle_OFF();
			break;	
