    ...
    if ( Usart_Num == Channel )
    {
        if ( temp_buffer[0] == NORMAL_DATA_SEND ) //发送数据类型
        {
            switch ( temp_buffer[1] )
            {
                ...
                case LIMIT_TEST_CMD:
                {
                    /* Limit switch test: backward to rear limit, then forward N pulses */
                    /* temp_buffer[2..3] = forward pulse count (big-endian u16) */
                    u16 fwd_pulses = ((u16)temp_buffer[2] << 8) | temp_buffer[3];
                    if (fwd_pulses == 0) fwd_pulses = 252; /* default: ~1% of full range */
                    motor_limit_test_start(fwd_pulses);
                    break;
                }
